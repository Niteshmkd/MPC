...
Gp = setmpcsignals(Gp,'MV',[1 2 3],'UD',[4 5]);
...
MPCobj.Model.Disturbance = [tf(1,[1 0]); tf(1,[1 0])];  %For unmeasured input disturbances, its model is an integrator
...
options.Unmeas = [(randn(Tf,1)) (randn(Tf,1))] ; % white noise
options.Model = Gp;
